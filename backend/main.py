...
SESSIONID = "70389442364%3AGxk54VEFLyVgjq%3A17%3AAYcBKF5WIK3fFBJiaTUl9DE1E7NX8UUzDWmxc3MkIw"
THREADID = None
VERBOSE = False
FILE_PATH = "/Users/rohansiva/Desktop/HackTX-24/MoodScribe/backend/dataFile.txt"
...
def get_threads():
    ...
    for thread in threads:
        if thread["is_group"]:
            name: str = thread.get('thread_title', 'Unnamed Group')
        else:
            # Handle cases where there might not be any users in the thread
            users = thread.get("users", [])
            if users:
                name: str = users[0].get("full_name", "Unknown User")
            else:
                name: str = "Empty Thread"
...
def main():
    ...
    with open(FILE_PATH, 'r') as file:
        ...
        output_directory = "/Users/rohansiva/Desktop/HackTX-24/MoodScribe/backend"
        file_path = os.path.join(output_directory, 'conversation.json')
